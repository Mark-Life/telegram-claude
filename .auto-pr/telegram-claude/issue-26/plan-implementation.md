...
- [x] **Task 1: Add `promptHistory` to UserState and create bounded storage helper**
  - Files: `src/bot.ts`
  - Changes:
...
- [x] **Task 2: Modify `buildPromptWithReplyContext` for reply-to-self detection**
  - Files: `src/bot.ts`
  - Changes:
...
- [x] **Task 3: Add `messageId` to `StreamResult` and `replyMarkup` option to `streamToTelegram`**
...
- [x] **Task 4: Pass `replyMarkup` through `safeEditMessage` to the final message edit**
  - Files: `src/telegram.ts`
  - Changes:
...
- [x] **Task 5: Wire up retry button in `handlePrompt` — create keyboard, pass to stream, store prompt after**
  - Files: `src/bot.ts`
  - Changes:
...
- [x] **Task 6: Add retry callback handler**
  - Files: `src/bot.ts`
  - Changes:
...
- [x] **Task 7: Verify everything works together**
