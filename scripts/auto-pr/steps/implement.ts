  ...
  for (let i = 1; i <= maxIterations; i++) {
    ...
    if (fileExists(completedPath)) {
      ...
      await commitArtifacts(
        ctx,
        `chore(auto-pr): implementation complete for ${ctx.repo}#${ctx.number}`
      );
      return true;
    }

    log(
      `Iteration ${i} finished but completed-summary.md not yet created — tasks remain`
    );
  }

  console.error(
    `Implementation did not complete after ${maxIterations} iterations`
  );
  return false;
}
