 ...
 */
export async function stepPlanImplementation(
  ctx: IssueContext
): Promise<boolean> {
  ...
  if (!fileExists(implPlanPath)) {
    console.error(
      "Plan-Implementation step did not produce plan-implementation.md"
    );
    return false;
  }
  ...
  await commitArtifacts(
    ctx,
    `chore(auto-pr): implementation plan for ${ctx.repo}#${ctx.number}`
  );
  return true;
}
